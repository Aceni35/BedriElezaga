...
function normalizeFile(f: File, allowedExtensions: Set<string>): File {
  const ext = getExtension(f.name);
  if (f.type && f.type !== 'application/octet-stream') return f;
  if (!allowedExtensions.has(ext)) return f;
...
function FileSlot({ label, hint, current, state, onChange }: FileSlotProps) {
  ...
  const handleFiles = (files: File[]) => {
    const raw = files[0];
    if (!raw) return;
    const f = normalizeFile(raw, ALLOWED_DOC_EXTENSIONS);
    const ext = getExtension(f.name);
    if (!ALLOWED_DOC_TYPES.has(f.type) && !ALLOWED_DOC_EXTENSIONS.has(ext)) {
